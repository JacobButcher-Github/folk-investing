// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: users.sql

package db

import (
	"context"
	"database/sql"
)

const createUser = `-- name: CreateUser :one
INSERT INTO
  users (user_login, hashed_password, dollars, cents)
VALUES
  (?, ?, ?, ?) RETURNING id, user_login, role, hashed_password, dollars, cents
`

type CreateUserParams struct {
	UserLogin      string `json:"user_login"`
	HashedPassword string `json:"hashed_password"`
	Dollars        int64  `json:"dollars"`
	Cents          int64  `json:"cents"`
}

func (q *Queries) CreateUser(ctx context.Context, arg CreateUserParams) (User, error) {
	row := q.db.QueryRowContext(ctx, createUser,
		arg.UserLogin,
		arg.HashedPassword,
		arg.Dollars,
		arg.Cents,
	)
	var i User
	err := row.Scan(
		&i.ID,
		&i.UserLogin,
		&i.Role,
		&i.HashedPassword,
		&i.Dollars,
		&i.Cents,
	)
	return i, err
}

const getUserFromId = `-- name: GetUserFromId :one
SELECT
  id, user_login, role, hashed_password, dollars, cents
FROM
  users
WHERE
  id = ?
LIMIT
  1
`

func (q *Queries) GetUserFromId(ctx context.Context, id int64) (User, error) {
	row := q.db.QueryRowContext(ctx, getUserFromId, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.UserLogin,
		&i.Role,
		&i.HashedPassword,
		&i.Dollars,
		&i.Cents,
	)
	return i, err
}

const getUserFromName = `-- name: GetUserFromName :one
SELECT
  id, user_login, role, hashed_password, dollars, cents
FROM
  users
WHERE
  user_login = ?
LIMIT
  1
`

func (q *Queries) GetUserFromName(ctx context.Context, userLogin string) (User, error) {
	row := q.db.QueryRowContext(ctx, getUserFromName, userLogin)
	var i User
	err := row.Scan(
		&i.ID,
		&i.UserLogin,
		&i.Role,
		&i.HashedPassword,
		&i.Dollars,
		&i.Cents,
	)
	return i, err
}

const updateUser = `-- name: UpdateUser :one
UPDATE users
SET
  user_login = COALESCE(?1, user_login),
  hashed_password = COALESCE(?2, hashed_password)
WHERE
  user_login = ?3 RETURNING id, user_login, role, hashed_password, dollars, cents
`

type UpdateUserParams struct {
	NewLogin       sql.NullString `json:"new_login"`
	HashedPassword sql.NullString `json:"hashed_password"`
	UserLogin      string         `json:"user_login"`
}

func (q *Queries) UpdateUser(ctx context.Context, arg UpdateUserParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateUser, arg.NewLogin, arg.HashedPassword, arg.UserLogin)
	var i User
	err := row.Scan(
		&i.ID,
		&i.UserLogin,
		&i.Role,
		&i.HashedPassword,
		&i.Dollars,
		&i.Cents,
	)
	return i, err
}
